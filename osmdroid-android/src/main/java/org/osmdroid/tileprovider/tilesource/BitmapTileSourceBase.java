...
public abstract class BitmapTileSourceBase implements ITileSource {
    ...
    public String getTileRelativeFilenameString(final long pMapTileIndex) {
        ...
        sb.append("L");
        if (zoom < 10) {
            sb.append("0").append(zoom);
        } else {
            sb.append(zoom);
        }
        sb.append('/');
        sb.append("R");
        sb.append(dealWithStr(tenToSix(y)));
        sb.append('/');
        sb.append("C");
        sb.append(dealWithStr(tenToSix(x)));
...
//
    ...
    private String tenToSix(int data) {
     ...
     * @return
     */
    private String dealWithStr(String data) {
        String[] res = new String[]{"0", "0", "0", "0", "0", "0", "0", "0"};
        for (int i = 1; i <= data.length(); i++) {
            res[8 - i] = String.valueOf(data.charAt(data.length() - i));
        }
...
//        System.out.println(dealWithStr("12"));
    }
